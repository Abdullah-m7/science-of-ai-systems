...
EXPECTED_CONTROLLER_PROTOCOL = "SMI-CP/003/1"
EXPECTED_CONTROLLER_CODE_SHA = "e9715e52f24465362d4b0768fc98e9551df2ed8a"
...
def extract_trial(
    ...
    if require_public_provenance:
        static_failures = []
        if ledger.get("protocol_version") != EXPECTED_CONTROLLER_PROTOCOL:
            static_failures.append("controller_protocol_mismatch")
        if reveal.get("protocol_version") != EXPECTED_CONTROLLER_PROTOCOL:
            static_failures.append("reveal_protocol_mismatch")
        if ledger.get("controller_code_sha") != EXPECTED_CONTROLLER_CODE_SHA:
            static_failures.append("controller_code_sha_mismatch")
        if reveal.get("controller_code_sha") != EXPECTED_CONTROLLER_CODE_SHA:
            static_failures.append("reveal_code_sha_mismatch")
        if str(ledger.get("subject_login")) != EXPECTED_SUBJECT_ACTOR:
            static_failures.append("subject_actor_mismatch")
        if static_failures:
            raise TrialIntegrityError(static_failures)
    ...
    if require_public_provenance:
        ...
        public_checks = verify_public_trial(
            ...
            expected_controller_actor=EXPECTED_CONTROLLER_ACTOR,
        )
...
def analyze_paths(
    ...
    *,
    ...
    duplicate_ids = sorted(key for key, count in valid_id_counts.items() if count > 1)
    if duplicate_ids:
    ...
    return {
        "analysis_version": ANALYSIS_VERSION,
        "status": status,
        "public_provenance_constraints": {
            "repository": EXPECTED_REPOSITORY,
            "controller_actor": EXPECTED_CONTROLLER_ACTOR,
            "subject_actor": EXPECTED_SUBJECT_ACTOR,
            "controller_protocol": EXPECTED_CONTROLLER_PROTOCOL,
            "controller_code_sha": EXPECTED_CONTROLLER_CODE_SHA,
        },
        "final_mode": final,
        "integrity": {
