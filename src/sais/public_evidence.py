...
DEFAULT_CONTROLLER_ACTOR = "github-actions[bot]"
...
def verify_public_trial(bundle: dict[str, Any]) -> dict[str, bool]:
    """Verify cryptographic trial content and its public issue/ledger provenance."""
    try:
        ledger = bundle["ledger"]
        reveal = bundle["reveal"]
        public = bundle["public_record"]
        comments = public["comments"]
        controller_actor = public.get("controller_actor")
...
def collect_public_trial(
    ...
    issue_number: int,
    token: str | None = None,
    controller_actor: str = DEFAULT_CONTROLLER_ACTOR,
) -> dict[str, Any]:
    ...
    try:
        if not controller_actor:
            raise ValueError("controller actor is missing")
        indexed = _indexed_controller_comments(comments, controller_actor)
    ...
    raw_ledger = base64.b64decode(content_record["content"])
    ...
    verification = verify_public_trial(bundle)
...
def main() -> None:
    ...
    parser.add_argument(
        "--controller-actor", default=DEFAULT_CONTROLLER_ACTOR,
        help="GitHub login that authored controller phase comments",
    )
    args = parser.parse_args()

    token = os.environ.get("GITHUB_TOKEN") or None
    bundle = collect_public_trial(
        args.repository, args.issue_number, token, args.controller_actor
    )
